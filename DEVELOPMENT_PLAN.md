...
## 首页下一阶段六项改造记录（2026-05-19）

以下 6 项为用户已确认要做的下一阶段改造需求，必须保留记录；在用户另行通知前，除第 4 项外，其余 5 项先不要动代码。
...
6. 增加流光效果与光粒效果；该项等待用户后续提供参考图后再实现。

当前执行策略：

- 当前只执行第 4 项。
- 第 1、2、3、5、6 项仅记录，不提前实现。
- 若后续继续修改 `DreamCorridor.jsx`，需先重读当前文件，因为用户可能会手动微调其中常量。

## 当前走廊待处理 / 待确认问题（2026-05-16）

以下问题已记录为本阶段优先项。在用户明确确认“已解决”前，下次继续问到“还有什么没解决”时，应继续按未确认项反馈：

1. 移动端走廊手势方向：**用户暂时认可当前修改**，但在最终整体验收前仍可继续观察。
2. 走廊滑动触感偏弱：**用户暂时认可当前修改**，但在最终整体验收前仍可继续观察。
3. 某些屏幕尺寸下走廊画框出画：**用户本轮确认当前状态没问题**。上一版基于屏幕宽高比收拢画框的改动已回退，不再使用。
4. 走廊卡顿排查：**用户本轮确认当前状态没问题**。贴图优化后延伸出的滚动偏快问题已通过下调鼠标滚轮速度和触摸速度处理。
5. 横屏提醒应覆盖整个网站：**当前用户认可**。
6. 从“进入深梦”进入三级页后，点击“回到走廊”应与聚焦页中的“返回走廊”效果一致：**用户已确认可以**。

备注：做完以上条目后，先停在本阶段，等待用户确认，再进入下一阶段。
...
## 走廊视觉基线参数（2026-05-23 用户确认）

以下参数为用户明确确认的“正确视觉”基线。后续若再次调整 `src/pages/DreamCorridor.jsx` 中的默认值、重置逻辑、localStorage 回退值或调试预设，必须先以这一组为准，不得再口头描述为“参数还在”但实际打开视觉不一致。
...
对应存储键：`dogdream:corridor-surface-wave-settings:v4`
...
对应存储键：`dogdream:corridor-smoke-settings:v3`
